...
SwitEntryPoint.test = function() {
    if ("undefined" === typeof window.SwitEntryFiles) SwitEntryPoint.include(SwitEntryPoint.BOOT_FILE);
        ...
        for (var a = window.SwitEntryFiles, b = 0; b < a.length; b++) SwitEntryPoint.include(a[b])
};
...
SwitEntryPoint.detectBrowser = function(a) {
    ...
    if (-1 != a.platform.indexOf("iPhone")) {
        if (SwitEntryPoint.infoBrowser.iDevice = !0, SwitEntryPoint.infoBrowser.touchDevice = !0, SwitEntryPoint.infoBrowser.platformType =
            "iPhone", /OS (\d+\_\d+)/.test(a.userAgent)) {
            var b = RegExp.$1,
                b = b.replace("_", ".");
...
SwitEntryPoint.isBrowserSupported = function() {
    for (var a = window.config, b = 0; b < a.browserSettings.length; b++) {
